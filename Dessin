...
public class Dessin extends JPanel {
	...
	public void paint(Graphics g) {
		...
		if(batiments != null) {
			for (Batiment b: batiments) {
				List<PointsB>points=b.getPointB();
				for (int i = 0 ; i < points.size() ; i = i + 1) {
					PointsB p1 = points.get(i) ;
					i ++ ;
					if (i < points.size()) {
						PointsB p2 = points.get(i+1) ;
	...
	public void paintG(Graphics g) {
		...
		List<Deplacement> deplNode0 = new ArrayList<Deplacement>();
		...
		//Création d'une liste des déplacements d'un véhicules
		List<Deplacement> deplNode1 = new ArrayList<Deplacement>();
		...
		//Création d'une liste des déplacements d'un véhicules
		List<Deplacement> deplNode2 = new ArrayList<Deplacement>();
		...
		List<PointsB> pbatiment0 = new ArrayList<PointsB>();
